...
class Laporan extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('ajl/laporan/laporan_eff_perkonstruksi');
...
/*-----------------------------------------------------------------FUNGSI EFF PERBULAN-----------------------------------------------------*/
	...
	public function laporan_view()
	{
		...
		$data['data'] = $this->M_laporan->lap_view($tgl_awal, $tgl_akhir);
		$data['awal'] = $tgl_awal;
	 	$data['akhir'] = $tgl_akhir;
		$this->load->view('ajl/index');
        $this->load->view('ajl/sidebar_content');
		$this->load->view('ajl/laporan/lap_kons_perbulan_view', $data);
		$this->load->view('ajl/footer');
	}
	public function laporan_view_cetak()
	{
		...
		$tgl_awal =  $this->input->post('tgl_awal');
		$tgl_akhir =  $this->input->post('tgl_akhir');
		$data = $this->M_laporan->lap_view($tgl_awal, $tgl_akhir);
		...
		$objPHPExcel->getSheet(0)->mergeCells('A1:N1');
		$objPHPExcel->getActiveSheet()->SetCellValue('A1', "Laporan EFF");
		$objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->setName('Times New Roman');
		$objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->setSize(15);
		$objPHPExcel->getActiveSheet()->getStyle('A1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
		
		$objPHPExcel->getSheet(0)->mergeCells('A2:N2');
		$objPHPExcel->getActiveSheet()->SetCellValue('A2', indonesian_date($tgl_awal)." - ".indonesian_date($tgl_akhir));
		...
		$objPHPExcel->getActiveSheet()->getStyle('A'.$rowCount.':'.'N'.$rowCount)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
        $objPHPExcel->getActiveSheet()->getStyle('A'.$rowCount.':'.'N'.$rowCount)->getFill()->getStartColor()->setARGB('999999');

        //  Border header
        $objPHPExcel->getActiveSheet()->getStyle('A'.$rowCount.':'.'N'.$rowCount)->applyFromArray(array('borders' => array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN))));        

		$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, "KONSTRUKSI");
		$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, "JANUARI");
		$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, "FEBRUARI");
		$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, "MARET");
		$objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, "APRIL");
		$objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, "MEI");
		$objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, "JUNI");
		$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, "JULI");
		$objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, "AGUSTUS");
		$objPHPExcel->getActiveSheet()->SetCellValue('J'.$rowCount, "SEPTEMBER");
		$objPHPExcel->getActiveSheet()->SetCellValue('K'.$rowCount, "OKTOBER");
		$objPHPExcel->getActiveSheet()->SetCellValue('L'.$rowCount, "NOVEMBER");
		$objPHPExcel->getActiveSheet()->SetCellValue('M'.$rowCount, "DESEMBER");
		$objPHPExcel->getActiveSheet()->SetCellValue('N'.$rowCount, "RATA-RATA");
		$rowCount++;

		// set datatable 
		foreach($data as $value){
		    $objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $value->jns_konstruksi); 
		    $objPHPExcel->getActiveSheet()->setCellValue('B'.$rowCount, $value->jan);
		    $objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, $value->feb); 
		    $objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, $value->mar); 
		    $objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, $value->apr); 
		    $objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, $value->mei);
		    $objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, $value->jun);
			$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, $value->jul);
			$objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, $value->agt);
			$objPHPExcel->getActiveSheet()->SetCellValue('J'.$rowCount, $value->sep);
			$objPHPExcel->getActiveSheet()->SetCellValue('K'.$rowCount, $value->okt);
			$objPHPExcel->getActiveSheet()->SetCellValue('L'.$rowCount, $value->nov);
			$objPHPExcel->getActiveSheet()->SetCellValue('M'.$rowCount, $value->des);
			$objPHPExcel->getActiveSheet()->SetCellValue('N'.$rowCount, $value->rata2_);
		    $rowCount++;
		}

		//  Border Data		
		$objPHPExcel->getActiveSheet()->getStyle('A5:N'.($rowCount-1))->applyFromArray(array('borders' => array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN))));
        ...
        sleep(1);
        $filename = 'Laporan_Eff_perbulan '.date('d-m-Y H-i').'.xlsx';
...
/*-------------------------------------------------------------END OF FUNGSI EFF PERBULAN--------------------------------------------------------*/


/*----------------------------------------------------------------FUNGSI EFF PERTANGGAL----------------------------------------------------------*/
	public function laporan_kons_perhari()
	{
		$bulan =  $this->input->post('bulan');
		$tahun =  $this->input->post('tahun');
		$data['data'] = $this->M_laporan->lap_kons_perhari($bulan, $tahun);
		$data['bulan'] = $bulan;
	 	$data['tahun'] = $tahun;
		$this->load->view('ajl/index');
        $this->load->view('ajl/sidebar_content');
		$this->load->view('ajl/laporan/lap_kons_perhari_view', $data);
		$this->load->view('ajl/footer');
	}

	public function laporan_perhari_cetak()
	{
		include_once './assets/cexcel/Classes/PHPExcel.php';
		date_default_timezone_set('Asia/Jakarta');
		$objPHPExcel = new PHPExcel();
		...
		$bulan =  $this->input->post('bulan');
		$tahun =  $this->input->post('tahun');
		$data = $this->M_laporan->lap_kons_perhari($bulan, $tahun);
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('A1', "Laporan EFF Pertanggal");
		$objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->setName('Times New Roman');
		$objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->setSize(15);
		$objPHPExcel->getActiveSheet()->getStyle('A1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
        ...
        $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, "Excel2007");
        sleep(1);
        $filename = 'Laporan_eff_pertanggal '.date('d-m-Y H-i').'.xlsx';
        header("Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        header('Content-Disposition: attachment; filename="'.$filename.'"');
        header("Cache-Control: max-age=0");
        ob_clean();
...
/*-------------------------------------------------------------------FUNGSI LUSI PAKAN-------------------------------------------------------------*/
	...
	public function laporan_lspk()
	{
		$bulan =  $this->input->post('bulan');
		$tahun =  $this->input->post('tahun');
		$data['data_lusi'] = $this->M_laporan->lap_eff_lusi($bulan, $tahun);		
		$data['data_pakan'] = $this->M_laporan->lap_eff_pakan($bulan, $tahun);
		$data['bulan'] = $bulan;
	 	$data['tahun'] = $tahun;
		$this->load->view('ajl/index');
        $this->load->view('ajl/sidebar_content');
		$this->load->view('ajl/laporan/lap_lusipakan_view', $data);
		$this->load->view('ajl/footer');
	}
public function laporan_lspk_cetak()
	{
		include_once './assets/cexcel/Classes/PHPExcel.php';
		date_default_timezone_set('Asia/Jakarta');
		$objPHPExcel = new PHPExcel();
		...
		$bulan =  $this->input->post('bulan');
		$tahun =  $this->input->post('tahun');
		$data1 = $this->M_laporan->lap_eff_lusi($bulan, $tahun);		
		$data2 = $this->M_laporan->lap_eff_pakan($bulan, $tahun);
		...
		$rowcount2 = 4;
																	/*lusi*/
		$objPHPExcel->getSheet(0)->mergeCells('A1:AG1');
		$objPHPExcel->getActiveSheet()->SetCellValue('A1', "Laporan Putus Lusi & Pakan");
		$objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->setName('Times New Roman');
		$objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->setSize(15);
		$objPHPExcel->getActiveSheet()->getStyle('A1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('A4',"Putus Lusi");
		...
		$objPHPExcel->getActiveSheet()->getStyle('A'.$rowCount.':'.'AG'.$rowCount)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
        $objPHPExcel->getActiveSheet()->getStyle('A'.$rowCount.':'.'AG'.$rowCount)->getFill()->getStartColor()->setARGB('999999');
		$objPHPExcel->getActiveSheet()->getStyle('A5:AG5')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
        $objPHPExcel->getActiveSheet()->getStyle('A5:AG5')->getFill()->getStartColor()->setARGB('999999');
        //  Border header
        $objPHPExcel->getActiveSheet()->getStyle('A5:AG5')->applyFromArray(array('borders' => array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN)))); 
	
        $objPHPExcel->getSheet(0)->mergeCells('B5:AF5');
		$objPHPExcel->getActiveSheet()->getStyle('B5:AF5')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('A5', "KONSTRUKSI");
		...
		$objPHPExcel->getSheet(0)->mergeCells('AG5:AG6');
		$objPHPExcel->getActiveSheet()->getStyle('AG5:AG6')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
     	$objPHPExcel->getActiveSheet()->SetCellValue('AG5', "RATA-RATA");
		$rowCount++;
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('A'.$hasil3, "Putus Pakan");
		...
		$objPHPExcel->getActiveSheet()->getStyle('A'.$hasil4.':'.'AG'.$hasil4)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
        $objPHPExcel->getActiveSheet()->getStyle('A'.$hasil4.':'.'AG'.$hasil4)->getFill()->getStartColor()->setARGB('999999');
		$objPHPExcel->getActiveSheet()->getStyle('A'.$hasil2.':'.'AG'.$hasil2)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
        $objPHPExcel->getActiveSheet()->getStyle('A'.$hasil2.':'.'AG'.$hasil2)->getFill()->getStartColor()->setARGB('999999');
        //  Border header
        $objPHPExcel->getActiveSheet()->getStyle('A'.$hasil2.':'.'AG'.$hasil2)->applyFromArray(array('borders' => array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN)))); 
	
        $objPHPExcel->getSheet(0)->mergeCells('B'.$hasil4.':'.'AF'.$hasil4);
		$objPHPExcel->getActiveSheet()->getStyle('B'.$hasil4.':'.'AF'.$hasil4)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
     	$objPHPExcel->getActiveSheet()->SetCellValue('B'.$hasil4, "TANGGAL");

        $objPHPExcel->getSheet(0)->mergeCells('A'.$hasil4.':'.'A'.$hasil2);
		$objPHPExcel->getActiveSheet()->getStyle('A'.$hasil4.':'.'A'.$hasil2)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$objPHPExcel->getActiveSheet()->SetCellValue('A'.$hasil4, "KONSTRUKSI");

		$objPHPExcel->getActiveSheet()->SetCellValue('B'.$hasil2, "1");
		$objPHPExcel->getActiveSheet()->SetCellValue('C'.$hasil2, "2");
		$objPHPExcel->getActiveSheet()->SetCellValue('D'.$hasil2, "3");
		$objPHPExcel->getActiveSheet()->SetCellValue('E'.$hasil2, "4");
		$objPHPExcel->getActiveSheet()->SetCellValue('F'.$hasil2, "5");
		$objPHPExcel->getActiveSheet()->SetCellValue('G'.$hasil2, "6");
		$objPHPExcel->getActiveSheet()->SetCellValue('H'.$hasil2, "7");
		$objPHPExcel->getActiveSheet()->SetCellValue('I'.$hasil2, "8");
		$objPHPExcel->getActiveSheet()->SetCellValue('J'.$hasil2, "9");
		$objPHPExcel->getActiveSheet()->SetCellValue('K'.$hasil2, "10");
		$objPHPExcel->getActiveSheet()->SetCellValue('L'.$hasil2, "11");
		$objPHPExcel->getActiveSheet()->SetCellValue('M'.$hasil2, "12");
		$objPHPExcel->getActiveSheet()->SetCellValue('N'.$hasil2, "13");
		$objPHPExcel->getActiveSheet()->SetCellValue('O'.$hasil2, "14");
		$objPHPExcel->getActiveSheet()->SetCellValue('P'.$hasil2, "15");
		$objPHPExcel->getActiveSheet()->SetCellValue('Q'.$hasil2, "16");
		$objPHPExcel->getActiveSheet()->SetCellValue('R'.$hasil2, "17");
		$objPHPExcel->getActiveSheet()->SetCellValue('S'.$hasil2, "18");
		$objPHPExcel->getActiveSheet()->SetCellValue('T'.$hasil2, "19");
		$objPHPExcel->getActiveSheet()->SetCellValue('U'.$hasil2, "20");
		$objPHPExcel->getActiveSheet()->SetCellValue('V'.$hasil2, "21");
		$objPHPExcel->getActiveSheet()->SetCellValue('W'.$hasil2, "22");
		$objPHPExcel->getActiveSheet()->SetCellValue('X'.$hasil2, "23");
		$objPHPExcel->getActiveSheet()->SetCellValue('Y'.$hasil2, "24");
		$objPHPExcel->getActiveSheet()->SetCellValue('Z'.$hasil2, "25");
		$objPHPExcel->getActiveSheet()->SetCellValue('AA'.$hasil2, "26");
		$objPHPExcel->getActiveSheet()->SetCellValue('AB'.$hasil2, "27");
		$objPHPExcel->getActiveSheet()->SetCellValue('AC'.$hasil2, "28");
		$objPHPExcel->getActiveSheet()->SetCellValue('AD'.$hasil2, "29");
		$objPHPExcel->getActiveSheet()->SetCellValue('AE'.$hasil2, "30");
		$objPHPExcel->getActiveSheet()->SetCellValue('AF'.$hasil2, "31");

		$objPHPExcel->getSheet(0)->mergeCells('AG'.$hasil4.':'.'AG'.$hasil2);
		$objPHPExcel->getActiveSheet()->getStyle('AG'.$hasil4.':'.'AG'.$hasil2)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
     	$objPHPExcel->getActiveSheet()->SetCellValue('AG'.$hasil4, "RATA-RATA");
		$hasil2++;

		// set datatable 
		foreach($data2 as $value){
		    $objPHPExcel->getActiveSheet()->SetCellValue('A'.$hasil2, $value->jns_konstruksi); 
		    $objPHPExcel->getActiveSheet()->setCellValue('B'.$hasil2, $value->TGL_1);
		    $objPHPExcel->getActiveSheet()->SetCellValue('C'.$hasil2, $value->TGL_2); 
		    $objPHPExcel->getActiveSheet()->SetCellValue('D'.$hasil2, $value->TGL_3); 
		    $objPHPExcel->getActiveSheet()->SetCellValue('E'.$hasil2, $value->TGL_4); 
		    $objPHPExcel->getActiveSheet()->SetCellValue('F'.$hasil2, $value->TGL_5);
		    $objPHPExcel->getActiveSheet()->SetCellValue('G'.$hasil2, $value->TGL_6);
			$objPHPExcel->getActiveSheet()->SetCellValue('H'.$hasil2, $value->TGL_7);
			$objPHPExcel->getActiveSheet()->SetCellValue('I'.$hasil2, $value->TGL_8);
			$objPHPExcel->getActiveSheet()->SetCellValue('J'.$hasil2, $value->TGL_9);
			$objPHPExcel->getActiveSheet()->SetCellValue('K'.$hasil2, $value->TGL_10);
			$objPHPExcel->getActiveSheet()->SetCellValue('L'.$hasil2, $value->TGL_11);
			$objPHPExcel->getActiveSheet()->SetCellValue('M'.$hasil2, $value->TGL_12);
		    $objPHPExcel->getActiveSheet()->setCellValue('N'.$hasil2, $value->TGL_13);
		    $objPHPExcel->getActiveSheet()->SetCellValue('O'.$hasil2, $value->TGL_14); 
		    $objPHPExcel->getActiveSheet()->SetCellValue('P'.$hasil2, $value->TGL_15); 
		    $objPHPExcel->getActiveSheet()->SetCellValue('Q'.$hasil2, $value->TGL_16); 
		    $objPHPExcel->getActiveSheet()->SetCellValue('R'.$hasil2, $value->TGL_17);
		    $objPHPExcel->getActiveSheet()->SetCellValue('S'.$hasil2, $value->TGL_18);
			$objPHPExcel->getActiveSheet()->SetCellValue('T'.$hasil2, $value->TGL_19);
			$objPHPExcel->getActiveSheet()->SetCellValue('U'.$hasil2, $value->TGL_20);
			$objPHPExcel->getActiveSheet()->SetCellValue('V'.$hasil2, $value->TGL_21);
			$objPHPExcel->getActiveSheet()->SetCellValue('W'.$hasil2, $value->TGL_22);
			$objPHPExcel->getActiveSheet()->SetCellValue('X'.$hasil2, $value->TGL_23);
			$objPHPExcel->getActiveSheet()->SetCellValue('Y'.$hasil2, $value->TGL_24);
			$objPHPExcel->getActiveSheet()->SetCellValue('Z'.$hasil2, $value->TGL_25);
		    $objPHPExcel->getActiveSheet()->setCellValue('AA'.$hasil2, $value->TGL_26);
		    $objPHPExcel->getActiveSheet()->SetCellValue('AB'.$hasil2, $value->TGL_27); 
		    $objPHPExcel->getActiveSheet()->SetCellValue('AC'.$hasil2, $value->TGL_28); 
		    $objPHPExcel->getActiveSheet()->SetCellValue('AD'.$hasil2, $value->TGL_29); 
		    $objPHPExcel->getActiveSheet()->SetCellValue('AE'.$hasil2, $value->TGL_30);
		    $objPHPExcel->getActiveSheet()->SetCellValue('AF'.$hasil2, $value->TGL_31);
		    $objPHPExcel->getActiveSheet()->SetCellValue('AG'.$hasil2, $value->rata2_);
		    $hasil2++;
			}

		$ds = $rowCount + 3;	
		$objPHPExcel->getActiveSheet()->getStyle('A'.$ds.':'.'AG'.($hasil2-1))->applyFromArray(array('borders' => array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN))));
				// set autosize
		$objPHPExcel->getActiveSheet()->getColumnDimension('A')->setAutoSize(true);
		$objPHPExcel->getActiveSheet()->getColumnDimension('B')->setAutoSize(true);
		$objPHPExcel->getActiveSheet()->getColumnDimension('C')->setAutoSize(true);
        ...
        $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, "Excel2007");
        sleep(1);
        $filename = 'Laporan_pt_lusi_pakan '.date('d-m-Y H-i').'.xlsx';
        header("Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        header('Content-Disposition: attachment; filename="'.$filename.'"');
        header("Cache-Control: max-age=0");
        ob_clean();
...
/*-------------------------------------------------------------------------------------------------------------------------------------------------*/
	...
	public function laporan_permesin()
	{
        ...
        $this->load->view('ajl/sidebar');
        $this->load->view('ajl/sidebar_content');
		$this->load->view('ajl/laporan/laporan_eff_permesin');
		$this->load->view('ajl/footer');
	}
	...
	public function laporan_permesin_view()
	{
		$bulan =  $this->input->post('bulan');
		$tahun =  $this->input->post('tahun');
		$data['data_permesin'] = $this->M_laporan->lap_eff_permesin($bulan, $tahun);		
		$data['data_total'] = $this->M_laporan->lap_eff_total_kagrup_permesin($bulan, $tahun);
		$data['bulan'] = $bulan;
	 	$data['tahun'] = $tahun;
		$this->load->view('ajl/index');
        $this->load->view('ajl/sidebar_content');
		$this->load->view('ajl/laporan/lap_eff_permesin_view', $data);
		$this->load->view('ajl/footer');
	}
...
public function laporan_permesin_cetak()
	{
		...
		$bulan =  $this->input->post('bulan');
		$tahun =  $this->input->post('tahun');
		$data1 = $this->M_laporan->lap_eff_permesin($bulan, $tahun);		
		$data2 = $this->M_laporan->lap_eff_total_kagrup_permesin($bulan, $tahun);
        ...
        $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, "Excel2007");
        sleep(1);
        $filename = 'Laporan_Eff_Mesin '.date('d-m-Y H-i').'.xlsx';
        header("Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        header('Content-Disposition: attachment; filename="'.$filename.'"');
        header("Cache-Control: max-age=0");
        ob_clean();
...
/*-------------------------------------------------------------------------------------------------------------------------------------------------*/
	...
	public function laporan_montir()
	{
        ...
        $this->load->view('ajl/sidebar');
        $this->load->view('ajl/sidebar_content');
		$this->load->view('ajl/laporan/laporan_eff_montir');
		$this->load->view('ajl/footer');
	}
	...
	public function laporan_montir_view()
	{
		$bulan =  $this->input->post('bulan');
		$tahun =  $this->input->post('tahun');
		$data['data_montir'] = $this->M_laporan->lap_eff_montir($bulan, $tahun);		
		$data['bulan'] = $bulan;
	 	$data['tahun'] = $tahun;
		$this->load->view('ajl/index');
        $this->load->view('ajl/sidebar_content');
		$this->load->view('ajl/laporan/lap_eff_montir_view', $data);
		$this->load->view('ajl/footer');
	}
	...
	public function laporan_montir_cetak()
	{
		...
		$bulan =  $this->input->post('bulan');
		$tahun =  $this->input->post('tahun');
		$data = $this->M_laporan->lap_eff_montir($bulan, $tahun);
        ...
        $objPHPExcel->getSheet(0)->mergeCells('A4:A5');
		$objPHPExcel->getActiveSheet()->getStyle('A4:A5')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$objPHPExcel->getActiveSheet()->SetCellValue('A4', "RS");
		...
		// set datatable 
		foreach($data as $value){
		    $objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $value->rs); 
		    $objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, $value->nama_mtc); 
...
/*-------------------------------------------------------------END OF FUNGSI EFF MONTIR--------------------------------------------------------*/
	...
	public function laporan_weff()
	{
		$this->load->view('ajl/index');
        $this->load->view('ajl/sidebar');
        $this->load->view('ajl/sidebar_content');
		$this->load->view('ajl/laporan/laporan_weff');
		$this->load->view('ajl/footer');
	}

	public function laporan_weff_view()
	{
		$bulan =  $this->input->post('bulan');
		$tahun =  $this->input->post('tahun');
		$data['data'] = $this->M_laporan->lap_weff($bulan, $tahun);		
		$data['bulan'] = $bulan;
	 	$data['tahun'] = $tahun;
		$this->load->view('ajl/index');
        $this->load->view('ajl/sidebar_content');
		$this->load->view('ajl/laporan/lap_weff_view', $data);
		$this->load->view('ajl/footer');
	}

public function laporan_weff_cetak()
	{
		include_once './assets/cexcel/Classes/PHPExcel.php';
		date_default_timezone_set('Asia/Jakarta');
		$objPHPExcel = new PHPExcel();
		...
		$bulan =  $this->input->post('bulan');
		$tahun =  $this->input->post('tahun');
		$data = $this->M_laporan->lap_weff($bulan, $tahun);
		...
		$objPHPExcel->getSheet(0)->mergeCells('A1:J1');
		$objPHPExcel->getActiveSheet()->SetCellValue('A1', "Laporan WEFF");
		$objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->setName('Times New Roman');
		$objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->setSize(15);
		$objPHPExcel->getActiveSheet()->getStyle('A1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
		
		$objPHPExcel->getSheet(0)->mergeCells('A2:J2');
		$objPHPExcel->getActiveSheet()->SetCellValue('A2', indonesian_date($bulan)." - ".indonesian_date($tahun));
		$objPHPExcel->getActiveSheet()->getStyle('A2')->getFont()->setName('Times New Roman');
		$objPHPExcel->getActiveSheet()->getStyle('A2')->getFont()->setItalic(true);		
		$objPHPExcel->getActiveSheet()->getStyle('A2')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
		...
		$objPHPExcel->getActiveSheet()->getStyle('A'.$rowCount.':'.'J'.$rowCount)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
        $objPHPExcel->getActiveSheet()->getStyle('A'.$rowCount.':'.'J'.$rowCount)->getFill()->getStartColor()->setARGB('999999');

        //  Border header
        $objPHPExcel->getActiveSheet()->getStyle('A'.$rowCount.':'.'J'.$rowCount)->applyFromArray(array('borders' => array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN))));        

		$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, "TANGGAL");
		$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, "RS");
		$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, "WEFF");
		$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, "MESIN");
		$objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, "KONSTRUKSI");
		$objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, "COUNT WEFF");
		$objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, "EFF");
		$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, "PUTUS LUSI");
		$objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, "PUTUS PAKAN");
		$objPHPExcel->getActiveSheet()->SetCellValue('J'.$rowCount, "SUM OF GULUNG MTR");
		$rowCount++;

		// set datatable 
		foreach($data as $value){
		    $objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $value->tgl_input); 
		    $objPHPExcel->getActiveSheet()->setCellValue('B'.$rowCount, $value->rs);
		    $objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, $value->weff); 
		    $objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, $value->no_mesin); 
		    $objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, $value->jns_konstruksi); 
		    $objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, $value->count_weff);
		    $objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, $value->eff);
			$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, $value->putus_lusi);
			$objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, $value->putus_pakan);
			$objPHPExcel->getActiveSheet()->SetCellValue('J'.$rowCount, $value->gulung_mtr);
		    $rowCount++;
		}

		//  Border Data		
		$objPHPExcel->getActiveSheet()->getStyle('A5:J'.($rowCount-1))->applyFromArray(array('borders' => array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN))));
        ...
        $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, "Excel2007");
        sleep(1);
        $filename = 'Laporan_Weff '.date('d-m-Y H-i').'.xlsx';
        header("Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        header('Content-Disposition: attachment; filename="'.$filename.'"');
        header("Cache-Control: max-age=0");
        ob_clean();
